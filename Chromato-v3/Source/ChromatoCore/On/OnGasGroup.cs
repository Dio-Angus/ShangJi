...
namespace ChromatoCore.On
{
    ...
    /// 通道气采集组合
    /// </summary>
    public partial class OnGasGroup : OnGroupBase
    {
        ...
        private string _multiRegisterTime = string.Empty;
        #endregion
        ...
        public override void CreateLayer(ChannelID lf, UserType user, CastPipe pipe)
        {
            ...
            switch (General.ObjectLink)
            {
                ...
                case General.LinkObject.AutoChromatoGas:
                    ...
                    {
                        case ChannelID.A:
                            CommPort.Instance.GetTransBiz(IdChannel.Tcd1).ShowVoltageActioned += new EventHandler<OnVoltageActionArgs>(this.ShowVoltageActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Tcd1).AutoSlopeActioned += new EventHandler<OnAutoSlopeActionArgs>(this.AutoSlopeActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Tcd1).AutoStopActioned += new EventHandler<OnAutoStopActionArgs>(this.AutoStopActioned);
                            break;
                        
                        case ChannelID.B:
                            CommPort.Instance.GetTransBiz(IdChannel.Fid1).ShowVoltageActioned += new EventHandler<OnVoltageActionArgs>(this.ShowVoltageActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Fid1).AutoSlopeActioned += new EventHandler<OnAutoSlopeActionArgs>(this.AutoSlopeActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Fid1).AutoStopActioned += new EventHandler<OnAutoStopActionArgs>(this.AutoStopActioned);
                            break;

                        case ChannelID.C:
                            CommPort.Instance.GetTransBiz(IdChannel.Tcd2).ShowVoltageActioned += new EventHandler<OnVoltageActionArgs>(this.ShowVoltageActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Tcd2).AutoSlopeActioned += new EventHandler<OnAutoSlopeActionArgs>(this.AutoSlopeActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Tcd2).AutoStopActioned += new EventHandler<OnAutoStopActionArgs>(this.AutoStopActioned);
                            break;

                        case ChannelID.D:
                            CommPort.Instance.GetTransBiz(IdChannel.Fid2).ShowVoltageActioned += new EventHandler<OnVoltageActionArgs>(this.ShowVoltageActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Fid2).AutoSlopeActioned += new EventHandler<OnAutoSlopeActionArgs>(this.AutoSlopeActioned);
                            CommPort.Instance.GetTransBiz(IdChannel.Fid2).AutoStopActioned += new EventHandler<OnAutoStopActionArgs>(this.AutoStopActioned);
        ...
        {
            ParaDto dto = null;
            CastLog.Logger("OnGasGroup", "StopSample", "Started");
            ...
            CastLog.Logger("OnGasGroup", "StopSample", "Finished");
        }
        ...
        private void ApplyRunThread()
        {
            while(true)
            {
                ...
                {
                    ParaDto dto = (ParaDto)this._arrApplyStop[0];

                    // 匿名代理
                    CrossThreadOperationControl CrossApplyAutoStop = delegate()
                    {
                        this.ApplyStopSample(dto);
                        this._arrApplyStop.Remove(dto);
                    };
                    if (this.InvokeRequired)
                    {
        ...
        private void GcStart()
        {
            ...
            {
                if (dto.ChannelID.Equals(GasChannel.A) && GcChannel.Tcd1)
                {
                    this._arrApplyRun.Add(dto);
                    arrTemp.Add(dto);
                }
                if (dto.ChannelID.Equals(GasChannel.B) && GcChannel.Fid1)
                {
                    this._arrApplyRun.Add(dto);
                    arrTemp.Add(dto);
                }
                if (dto.ChannelID.Equals(GasChannel.C) && GcChannel.Tcd2)
                {
                    this._arrApplyRun.Add(dto);
                    arrTemp.Add(dto);
                }
                if (dto.ChannelID.Equals(GasChannel.D) && GcChannel.Fid2)
                {
                    this._arrApplyRun.Add(dto);
                    arrTemp.Add(dto);
                }
        ...
        private void ApplyStopSample(ParaDto dto)
        {
            ...
            {
                case GasChannel.A:
                    this.StopChannel(dto, GasChannel.A, ChannelID.A, ChannelCmd.StopTcd1, ChannelCmd.StopBaseTcd1);
                    break;

                case GasChannel.B:
                    this.StopChannel(dto, GasChannel.B, ChannelID.B, ChannelCmd.StopFid1, ChannelCmd.StopBaseFid1);
                    break;
                
                case GasChannel.C:
                    this.StopChannel(dto, GasChannel.C, ChannelID.C, ChannelCmd.StopTcd2, ChannelCmd.StopBaseTcd2);
                    break;

                case GasChannel.D:
                    this.StopChannel(dto, GasChannel.D, ChannelID.D, ChannelCmd.StopFid2, ChannelCmd.StopBaseFid2);
                    break;
            }
        }
        ...
        /// </summary>
        /// <param name="dto"></param>
        /// <param name="gasChannel"></param>
        /// <param name="idChannel"></param>
        /// <param name="stopCmd"></param>
        /// <param name="stopBaseCmd"></param>
        private void StopChannel(ParaDto dto, String gasChannel, ChannelID idChannel, 
            ChannelCmd stopCmd, ChannelCmd stopBaseCmd)
        {
            RealStatus rs = CommPort.Instance.GetRealStatus(idChannel);
            ...
            switch (rs)
            {
                case RealStatus.Ready:
                    ...
                    //MessageBox.Show(gasChannel + "通道已经停止！", "提示");
                    ///break;
                ...
                case RealStatus.RealCollecting:
                    ...
                    this._bottomOn._viewGraph.SetChannelText(idChannel, 
                        CommPort.Instance.GetRealStatus(idChannel), dto.SampleName);
        ...
        private void ApplyRunSample(ParaDto dto)
        {
            ...
            {
                case GasChannel.A:
                    this.RunChannel(dto, GasChannel.A, ChannelID.A, ChannelCmd.StartTcd1);

                    break;

                case GasChannel.B:
                    this.RunChannel(dto, GasChannel.B, ChannelID.B, ChannelCmd.StartFid1);
                    break;
                
                case GasChannel.C:
                    this.RunChannel(dto, GasChannel.C, ChannelID.C, ChannelCmd.StartTcd2);
                    break;

                case GasChannel.D:
                    this.RunChannel(dto, GasChannel.D, ChannelID.D, ChannelCmd.StartFid2);
                    break;
            }
        }
        ...
        private void RunChannel(ParaDto dto, String gasChannel, ChannelID id, ChannelCmd startCmd)
        {
            switch (CommPort.Instance.GetRealStatus(id))
            {

                case RealStatus.SimuCollecting:
                    MessageBox.Show(gasChannel + "通道正在模拟采集，请启动其它通道或停止！", "提示");
                    break;

                case RealStatus.RealCollecting:
                    //MessageBox.Show(gasChannel + "通道正在采集，请启动其它通道或停止！", "提示");
                    break;

                case RealStatus.StopBase:
                    //MessageBox.Show(gasChannel + "通道正在停止中，请启动或停止其它通道！", "提示");
                    break;

                case RealStatus.ManulStart:
                    //MessageBox.Show(gasChannel + "通道正在启动中，请启动或停止其它通道！", "提示");
                    break;

                case RealStatus.Ready:
                //MessageBox.Show("Tcd1通道请走基线！", "提示");
                //break;
                case RealStatus.RunBase:
                case RealStatus.ManulStop:
                    ...
                    this.AddParaToRunningList(dto);
                    this._bottomOn._viewGraph.SetChannelText(id, CommPort.Instance.GetRealStatus(id), dto.SampleName);
                    break;
            }
        }
        ...
        private void InsertPara(ref ParaDto dto)
        {
            ...
            dto.RegisterTime = this._multiRegisterTime;
            dto.UserID = "cai";
        ...
        private void ApplyRunBase(ParaDto dto)
        {
            ...
            {
                case GasChannel.A:
                    this.RunBaseChannel(dto, GasChannel.A, ChannelID.A, ChannelCmd.RunBaseTcd1);
                    break;

                case GasChannel.B:
                    this.RunBaseChannel(dto, GasChannel.B, ChannelID.B, ChannelCmd.RunBaseFid1);
                    break;
                
                case GasChannel.C:
                    this.RunBaseChannel(dto, GasChannel.C, ChannelID.C, ChannelCmd.RunBaseTcd2);
                    break;

                case GasChannel.D:
                    this.RunBaseChannel(dto, GasChannel.D, ChannelID.D, ChannelCmd.RunBaseFid2);
                    break;
            }
        }
        ...
        /// <param name="gasChannel"></param>
        /// <param name="id"></param>
        /// <param name="startBaseCmd"></param>
        private void RunBaseChannel(ParaDto dto, String gasChannel, ChannelID id, ChannelCmd startBaseCmd)
        {
            switch (CommPort.Instance.GetRealStatus(id))
            {
                case RealStatus.Ready:
                ...
                case RealStatus.SimuCollecting:
                    MessageBox.Show(gasChannel + "通道正在模拟采集！", "提示");
                    break;

                case RealStatus.RealCollecting:
                    //MessageBox.Show(gasChannel + "通道正在采集！", "提示");
                    break;

                case RealStatus.ManulStop:
                case RealStatus.StopBase:
                    //MessageBox.Show(gasChannel + "通道正在停止中！", "提示");
                    break;

                case RealStatus.ManulStart:
                    //MessageBox.Show(gasChannel + "通道正在启动中！", "提示");
                    break;

                case RealStatus.RunBase:
                    //MessageBox.Show(gasChannel + "通道正在走基线！", "提示");
                    break;
            }
        }
        ...
        public override void InitAuto(AutoRequest auto)
        {
            switch (General.ObjectLink)
            {
                case General.LinkObject.AutoChromatoGas:
                    ...
                    if (null != auto)
                    {
                        ...
                        this._autoQuest.ChannelActioned += new EventHandler<OnChannelActionArgs>(listOn_ChannelActioned);
                    }
                    break;
            }
        ...
        private void listOn_ChannelActioned(object sender, OnChannelActionArgs e)
        {
            ...
            //注册时间自动设置为相同
            this._multiRegisterTime =  DateTime.Now.ToString("yyyyMMddHHmmss");

            if (ShowChannel.Syn)
            {
                arr = this._bizPara.LoadAllChannelPara(e._dtoPara);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public override void GasBoard_StartAutoSample(object sender, OnStartArgs e)
        {
            ...
            foreach (ParaDto dto in this._arrRunBase)
            {
                ...
                {
                    this._arrApplyRun.Add(dto);
                    CastLog.Logger("OnGasGroup", "GasBoard_StartAutoSample", dto.ChannelID);
                    break;
                }
            }
        }
