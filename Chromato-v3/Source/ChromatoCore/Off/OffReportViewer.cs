...
namespace ChromatoCore.Off
{
    ...
    /// 汇总报告
    /// </summary>
    public partial class OffReportViewer : UserControl
    {
        ...
        /// 样品dto列表
        /// </summary>
        private ArrayList _arrPara = null;
        ...
        private void SetupReport()
        {
            ...
            //{
            //    this._dsResult = this._bizPeak.LoadPrintPeakResult(this._dtoPara.PathData);
            //}
        }
        ...
        /// <param name="dt"></param>
        public void LoadUi(ArrayList arr, DataTable dt)
        {
            this._arrPara = arr;
            this._dtResult = dt;
        ...
        {
            if (null == this._arrPara || 0 == this._arrPara.Count)
            {
                return;
            }
            ...
            foreach (ParaDto dto in this._arrPara)
            {
                count++;
                if (1 == this._pageNo)
                {
                    ...
                    if (1 == count)
                    {
                        this.PrintInfo(e, ref currentPosition, dto);
                    }
                    //this.PrintGraph(e, ref currentPosition, dto);
                }

                if (null == this._dtResult)
                {
            ...
            this.printDataTable(this._dtResult, e, Convert.ToInt32(currentPosition));
        }
        ...
        private void PrintGraph(PrintPageEventArgs e, ref float currentPosition, ParaDto dto)
        {
            try
            {
                String dir = Application.ExecutablePath;
                int lastindex = dir.LastIndexOf('\\');
                String path = dir.Substring(0, lastindex + 1) + "Bmp\\" + dto.RegisterTime.Substring(0, 6) + "\\"
                    + dto.SampleName
                    + "_" + dto.RegisterTime
                    + "_" + dto.ChannelID + ".bmp";
        ...
        /// <param name="currentPosition"></param>
        /// <param name="dto"></param>
        private void PrintInfo(PrintPageEventArgs e, ref float currentPosition, ParaDto dto)
        {
            String fontName = "宋体";
            ...
            String sampleName = String.Format("样品名：{0}", dto.SampleName);
            ...
            //输出检测器类型和检测器参数
            String detector = String.Format("检测器：{0}", dto.ChannelID);
            String para = "";
            ...
            //输出样品类型和进样口温度
            String sampleType = String.Format("样品类型：{0}",EnumDescription.GetFieldText(dto.SampleType));
